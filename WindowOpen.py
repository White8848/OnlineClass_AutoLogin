...
def update(tx1, tx2, tx3, tx4,list1):
    tx1.delete(0, 'end')
    tx2.delete(0, 'end')
    tx3.delete(0, 'end')
    tx4.delete(0, 'end')
...
def main():
    ...
    # 第3步，设定窗口的大小(长 * 宽)
    window.geometry('385x200')  # 这里的乘是小x

    # 输入信息
    tk.Label(window, text='Class Name:', font=('Arial', 10)).place(x=50, y=5)
    tk.Label(window, text='Class ID:', font=('Arial', 10)).place(x=50, y=35)
    tk.Label(window, text='Account:', font=('Arial', 10)).place(x=50, y=60)
    tk.Label(window, text='Password:', font=('Arial', 10)).place(x=50, y=85)

    tk.Label(window, text='Author:Yirule', font=('Arial', 10)).place(x=300, y=170)
    # 课程列表
    lb = tk.Listbox(window, listvariable=None, width=4, height=6)
    lb.place(x=5, y=5)
    # lb.selection_set(0)
    ...
    entry_class_name = tk.Entry(window, textvariable=var_class_name, font=('Arial', 10))
    entry_class_name.place(x=135, y=5)
    # 课程ID
    var_class_id = tk.StringVar()
    var_class_id.set(user_data[1])
    entry_class_id = tk.Entry(window, textvariable=var_class_id, font=('Arial', 10))
    entry_class_id.place(x=135, y=35)
    # 用户名
    var_account = tk.StringVar()
    var_account.set(user_data[2])
    entry_account = tk.Entry(window, textvariable=var_account, font=('Arial', 10))
    entry_account.place(x=135, y=60)
    # 用户密码
    var_password = tk.StringVar()
    var_password.set(user_data[3])
    entry_password = tk.Entry(window, textvariable=var_password, font=('Arial', 10))
    entry_password.place(x=135, y=85)

    log = tk.Button(window, text='Login', font=('Arial', 12), width=8, height=1,
                    command=lambda: au.Subject.login(subject[lb.curselection()]))
    log.place(x=290, y=5)
    ...
    add.place(x=290, y=50)
    up = tk.Button(window, text='Update', font=('Arial', 12), width=8, height=1,command=lambda: update(
        entry_class_name, entry_class_id,entry_account, entry_password, lb))
    up.place(x=290, y=100)

    # user_data[0] = subject[lb.curselection()].class_name
